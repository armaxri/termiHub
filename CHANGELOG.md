...
- Network tools: pinging a host that fails to resolve (or otherwise errors) no longer leaves the **Ping** panel stuck on "running" with a Stop button that does nothing. The backend already emitted a `network-ping-error` event on a fatal error (e.g. DNS failure) and then ended the task, but the panel never listened for it — so it stayed "running" forever while the underlying task was already gone, making Stop a no-op. The panel now handles the error event: it leaves the running state, shows the error message, and tears down its listeners. Relatedly, stopping a running ping is now correctly reported as **canceled** instead of **completed** (the cancellation flag was previously read before the ping loop ran, so it was always `false`).
