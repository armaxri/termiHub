...
- Terminal: the rightmost character column is no longer missing. FitAddon reserves `overviewRuler.width || 14` pixels on the right of the viewport when computing the column count, but our slim 5 px slider (plus 2 px inset) only needs 7 px — the extra 7 px left an empty character-width strip between the rendered content and the slider that visually read as a hidden column. Pin `overviewRuler.width` to 7 px so FitAddon reservation matches the actual slider geometry.
