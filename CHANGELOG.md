...
- Terminal: the rightmost column is no longer partially hidden behind the scrollbar slider on first render. The Nerd Font ships via `@font-face` with `font-display: swap`, so xterm's initial `FitAddon.fit()` ran against the fallback monospace's cell width; when the real font swapped in, content reflowed slightly wider than the reserved viewport and the last column ended up under the slider (until a manual window resize triggered a fresh fit). The terminal now re-fits once `document.fonts.ready` resolves.
