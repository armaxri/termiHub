...
- Terminal: configurable **line endings** (PuTTY-style). A new **Settings → Terminal → Line Ending (Enter & Paste)** dropdown (LF / CR / CRLF, default **LF**) controls the byte sequence sent when pressing Enter and is used to normalize line endings in pasted text. This fixes the long-standing bug where pasting multi-line Windows (CRLF) text into a Unix SSH or serial session inserted a blank line between every row. The setting applies uniformly to local, SSH, serial, and telnet terminals and can be overridden per connection (**Connection → Terminal → Line Ending**, with a "use global default" option).
