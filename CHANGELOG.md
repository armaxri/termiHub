...
- Testing: began porting the WebdriverIO E2E suites onto the Python bridge harness, starting with **connection CRUD** (`tests/system/tests/test_connection_crud.py`, replacing `tests/e2e/connection-crud.test.js`): create/edit/delete/duplicate connections, folder creation, editor-as-tab behavior, duplicate-name validation, the ping context menu, SSH creation, and the export dialog. To drive these the in-app test bridge gained three verbs — **`contextMenu`** (open a right-click menu), **`selectOption`** (choose a `<select>` option), and **`pressKey`** (dispatch a key such as `Escape`) — and `click` now emulates a full pointer sequence so pointer-driven menus (the Radix Settings gear `DropdownMenu`) open. UI elements with UUID `data-testid`s (connections, folders, tabs) are resolved by **name** through `getState` via a new `termihub_harness.ui` module (`find_connection`, `find_folder`, `find_tab`, `tab_count`, …) and a `ConnectionsUi` suite mixin. [docs/test-bridge.md](docs/test-bridge.md) (#807)
