...
- Testing: the test bridge gained two verbs the UI port needed — **`drag`** and **`getComputedStyle`** — and the first WebdriverIO UI specs were ported onto the Python harness. `drag` (`{ action: "drag", testId, dx, dy? }`) performs a pointer drag by dispatching `mousedown` on the element then `mousemove`/`mouseup` on the document offset by the delta — driving handlers like the sidebar resize that `click` could not. `getComputedStyle` (`{ action: "getComputedStyle", testId?, property }`) reads a _computed_ CSS property, including theme custom properties from the document root (`--bg-primary`, …), which `getAttribute` cannot see. Both are available on the TS `Driver`, the Python `Driver` (`drag`, `get_computed_style`), and the declarative scenario runner (a `drag` step and a `computedStyleEquals` check). The **sidebar toggle**, **sidebar resize**, and **split-view** E2E suites now run via the Python harness (`tests/system/tests/`) with assertions mapped to store state (`sidebarCollapsed`, `rootPanel`, `sidebarWidth`); the original WebdriverIO specs were removed. [docs/test-bridge.md](docs/test-bridge.md) (#808)
