...
- Terminal: the built-in xterm.js 6 scrollbar now matches the rest of the UI. The slider color is sourced from the active theme's `scrollbarThumb` / `scrollbarThumbHover` tokens (previously xterm fell back to its own foreground-derived defaults, which clashed with the UI), the slider is rounded and inset from the right edge so it reads like an overlay scrollbar, has a `min-height` so deep scrollback buffers don't shrink it to a small square at the bottom of the track, and the VS Code-inherited inset shadow artifacts (top stripe, left stripe, and the 3×3 top-left corner square) are hidden.
- Terminal: the rightmost character column is no longer missing. FitAddon hardcodes a 14 px reservation on the right of the viewport for the scrollbar, but our slim 5 px slider (plus 2 px inset) only needs 7 px — the extra 7 px left an empty character-width strip between the rendered content and the slider that visually read as a hidden column. Replace `FitAddon.fit()` with a custom `fitTerminal` helper that uses a matching 7 px reservation, so the column count picks up the column FitAddon was discarding (verified via DOM measurement). FitAddon is still kept for `proposeDimensions()` in horizontal-scroll mode.
