...
- **Layout**: the custom `react-resizable-panels` + dnd-kit split-view system has been replaced by [`flexlayout-react`](https://github.com/caplin/FlexLayout) 0.9. Tab groups now use a `flexlayout-react` `Model` (serialised as `IJsonModel`) instead of a custom `PanelNode` tree. The flat `TabGroup.tabs` array replaces the nested panel tree; tab IDs are used directly as flexlayout node IDs. This brings built-in drag-and-drop reordering, split creation via DnD, and proper multi-tabset layouts with no custom tree-management code. Closes #730.
