...
export interface AppState
  ...
  //
  // The transient `transfers` map + `applyTransferProgress` / `cancelTransfer`
  // and the Transfer Queue panel mutations (`removeTransfer` / `clearCompleted`
  // / `setTransferQueueMinimized`) are provided by createTransfersSlice
  ...
  // File browser (SFTP + local) — navigate/refresh/mode/clipboard actions plus the
  // per-client `sessionFileBrowserId` pointer live in FileBrowsersSlice
  ...
  // cancelWorkflowRun / dismissWorkflowRunOutput / localProcessPrompt /
  // resolveLocalProcessPrompt) live in WorkflowsSlice (extracted under #2077 via
  // #2881). The in-flight run + inline output panel are region-authoritative (#2206).
...
export const useAppStore = create<AppState>((set, get, store) => {
  ...
  return {
    ...
    // panel mutations (`removeTransfer` / `clearCompleted` /
    // `setTransferQueueMinimized`) are provided by createTransfersSlice
    // (ARCH-001/FES-011, extracted under #2077 via #2881).
    ...
    // File browser (SFTP + local) — navigate/refresh/mode/clipboard actions and
    // `sessionFileBrowserId` provided by createFileBrowsersSlice (ARCH-001/FES-011,
    // the first cut of the appStore god-module split). The view stays
