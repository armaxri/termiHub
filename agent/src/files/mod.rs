...
pub trait FileBackend: Send + Sync {
    ...
    fn list(
        &self,
        path: &str,
    ) -> impl std::future::Future<Output = Result<Vec<FileEntry>, FileError>> + Send;

    /// Read file content, returning raw bytes.
    fn read(
        &self,
        path: &str,
    ) -> impl std::future::Future<Output = Result<Vec<u8>, FileError>> + Send;
    ...
    fn write(
        ...
        path: &str,
        data: &[u8],
    ) -> impl std::future::Future<Output = Result<(), FileError>> + Send;
    ...
    fn stat(
        &self,
        path: &str,
    ) -> impl std::future::Future<Output = Result<FilesStatResult, FileError>> + Send;
}
