...
status: partial
resolution: "#3116 — MAINTAINER decided ACCEPT+DOCUMENT (2026-09-19). Doc landed #3116 (docs/architecture.md §8, CSP & capability-scoping subsection); effortful post-release tightening → follow-up #3115. Fact-finding: CSP already well-hardened (tauri.conf.json:24 default-src 'self', no unsafe-eval, no inline-script, object-src/frame-src 'none', base-uri 'self', form-action 'none'). 3 flagged relaxations all JUSTIFIED: (1) script-src 'wasm-unsafe-eval' REQUIRED by Shiki Oniguruma WASM (monacoCustomLanguages→shiki/onig.wasm) — WASM-only, not JS eval; removable only by dropping syntax highlighting. (2) style-src 'unsafe-inline' + dangerousDisableAssetCspModification[style-src] forced by RUNTIME <style> injection from xterm/sonner/Monaco (documented commit 9f4594ab; Tauri auto-hash of index.html cancels 'unsafe-inline' per CSP3); OUR theming is CSSOM setProperty (exempt) — nonce/hash migration high-effort+fragile (no stable hash set, no Tauri runtime-nonce hook). (3) fs:* + opener:allow-open-path unscoped because users pick arbitrary export/open paths via OS dialog; URL-open already allowlisted (safeOpenExternal http/https/mailto). Action: document the rationale (#TBD doc PR) + file post-release follow-up for effortful tightening (style-src nonce, fs/opener scoping). Effortful tightening DEFERRED post-release (low payoff)"
---
