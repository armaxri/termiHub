...
# Include the VNC server (requires vnc profile) + drive the vnc backend
docker compose -f tests/docker/docker-compose.yml up -d vnc-server   # or: --profile vnc up -d
...
| VNC                    | `core/tests/vnc.rs`                                 | vnc-server:2501 (profile `vnc`)            | Live RFB path of the `vnc` graphical backend (#1681/#1713) against a real x11vnc server serving a static four-quadrant pattern: connect + VncAuth, decode a real framebuffer end to end (asserts each quadrant's colour), input/clipboard round-trip over the wire, and wrong-password rejection. Requires the `vnc` compose profile; port via `TERMIHUB_TEST_VNC_PORT` (default 2501) — see [Parallel test isolation](#parallel-test-isolation)                                                                                                                                                   |
...
| VNC host port                    | `2501`                             | `base + test_port_offset`, published by `tests/docker/docker-compose.yml`.    |
