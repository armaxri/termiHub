...
| ------------------------------------------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------- |
| [in-field-update-mechanism.html](partial/in-field-update-mechanism.html) | Variant A (desktop self-update): version detection, notification popup, status-bar indicator, Settings page, skip/security bypass | Release-notes security marker (#1878); Variants B/C download-and-prompt / auto-install flagged as a code-signing maintainer decision           |
| [light-color-theme.html](partial/light-color-theme.html)                 | Dark/Light/System + Solarized theme engine, CSS variables, live xterm.js re-theming                                               | Custom theme editor + `customThemes` field (#1879), theme import/export (#1880)                                                                |
| [persistent-connection-ux.html](partial/persistent-connection-ux.html)   | Full Start/Attach/Stop lifecycle, state store, IPC + events, agent-hosted sidebar UX and daemon persistence (#672/#709/#767)      | Desktop-local sidebar surfacing (#1881); status-bar background-session count (#1882)                                                           |
| [session-auto-save.html](partial/session-auto-save.html)                 | Automatic last-session persistence + restore-on-startup shipped (#586)                                                            | Session history / Recent Sessions / quick-connect (#1883); ask/never/always restore mode + dialog (#1884)                                      |
| [ui-modernization.html](partial/ui-modernization.html)                   | Primitive layer, toast hub, tokens, and the full dialog/form migration all shipped and closed (#1059–#1063)                       | Residual bespoke `__btn` classes in ~15 non-dialog surfaces (#1885); folder move to `implemented/` is a maintainer call                        |
| [confirm-dialog-primitive.html](partial/confirm-dialog-primitive.html)   | `ConfirmDialog` primitive in `src/components/ui/` with a `ConfirmDontAskAgain` opt-out, used by several dialogs                   | Named call sites still on `Modal`+`Button` not `ConfirmDialog` — body slot (#1875), migration (#1876), Port Scanner "don't warn again" (#1877) |
