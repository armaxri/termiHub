...
Fully implemented.
The `termihub-core` crate (`core/`) is production-ready and in use by both the desktop app (`src-tauri/`) and the remote agent (`agent/`). It provides the shared buffer (RingBuffer), config types, error types, file backend trait, monitoring parsers, output coalescer, protocol types, and session transport traits.
