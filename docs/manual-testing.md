...
Manual test procedures for features that cannot be effectively automated. Run these tests before releases and after major changes. For automated testing (unit, integration, E2E), see [Testing Strategy](testing.md).
...
- For serial port tests, set up virtual serial ports with `socat` (see `examples/` and [Serial Setup](serial-setup.md))
