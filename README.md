...
- **Credential vault backup** — Export saved credentials to a passphrase-protected, encrypted file (Settings → Security → Export vault…) and import it on another machine or after a reinstall (Import vault…)
...
- **Run on many terminals** — "Run on…" in the Workflows panel runs a workflow on a chosen set of connected terminals (or the current broadcast group), one after another, with a per-terminal summary
