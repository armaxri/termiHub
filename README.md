...
- **Backup & restore** — Back up all app data (connections, settings, themes, keyboard shortcuts, workspaces, macros, workflows, tunnels, embedded servers, network tools, trusted SSH host keys and RDP certificates, installed plugins, and optionally the encrypted credential vault) to one passphrase-protected file and restore it — per part, merged or replaced (Settings → Backup & Restore)
