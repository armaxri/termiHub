...
pub fn expand_config_value(value: &str) -> String {
    expand_tilde(&expand_env_placeholders(value))
}
...
mod tests {
    ...
    #[test]
    fn expand_config_value_with_env_and_tilde() {
        env::set_var("TERMIHUB_TEST_DIR", "projects");
        let result = expand_config_value("~/${env:TERMIHUB_TEST_DIR}/app");
        assert!(!result.starts_with('~'), "tilde should be expanded");
        assert!(
            result.ends_with("/projects/app"),
            "expected path ending in /projects/app, got: {result}"
        );
        env::remove_var("TERMIHUB_TEST_DIR");
    }

    #[test]
    fn expand_config_value_plain_text() {
        assert_eq!(expand_config_value("plain text"), "plain text");
    }

    #[test]
    fn expand_config_value_tilde_only() {
        let result = expand_config_value("~/docs");
        assert!(!result.starts_with('~'));
        assert!(result.ends_with("/docs"));
    }

    #[test]
    fn expand_config_value_env_only() {
        env::set_var("TERMIHUB_TEST_PATH", "/opt/bin");
        assert_eq!(expand_config_value("${env:TERMIHUB_TEST_PATH}"), "/opt/bin");
        env::remove_var("TERMIHUB_TEST_PATH");
    }
}
