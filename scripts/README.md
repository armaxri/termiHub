...
| `run-guided-manual`       | Starter for the **guided-manual** operator suite (`--manual`) — builds if stale, brings up the SSH fixtures, and tees the full transcript to a gitignored `tests/reports/manual-run.log` (the X11 display is resolved by the harness, not the script) |
