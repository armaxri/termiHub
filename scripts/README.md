...
| `coverage`                | Unified whole-app coverage — runs frontend (vitest/v8) + Rust (cargo-llvm-cov), merges both lcov files, and prints ONE repo-wide number. Advisory (reports, does not gate). Needs `cargo-llvm-cov`. `--dry-run` skips the heavy runs and only exercises the merge/summary                                                                                                                      |
| `check`                   | Read-only quality checks mirroring CI (formatting, linting, clippy)                                                                                                                                                                                                                                                                                                                            |
| `ci-local`                | **Reproduce the whole per-PR CI gate with one command** — mirrors `.github/workflows/code-quality.yml` (composes `check`, then adds tsc, per-feature core builds, `cargo audit`/`cargo deny`, the pnpm prod-audit gate, the Python machinery suite, plugin packaging, and commitlint). `--quick` runs the quality-only subset (what the pre-push hook uses). A clean run means a green PR gate |
...
# Before pushing
./scripts/format.sh
./scripts/ci-local.sh            # reproduce the full per-PR CI gate locally
# (or the pieces individually: ./scripts/test.sh + ./scripts/check.sh)
...
Committed git hooks live in [`hooks/`](hooks/) and are activated through git's
native `core.hooksPath` (no extra dependency). `./scripts/setup.sh` (and
`setup.cmd`) turns them on with `git config core.hooksPath scripts/hooks`; to
enable them in an existing clone without a full setup, run that one command
yourself. They are a per-clone setting, so they stay opt-in.

| Hook         | When         | What it does                                                                                      |
| ------------ | ------------ | ------------------------------------------------------------------------------------------------- |
| `pre-commit` | `git commit` | Prettier `--check` + ESLint on **staged** `src/`/`docs/` files only — fast, seconds               |
| `commit-msg` | `git commit` | Runs commitlint on the message (catches bad type/case and over-long headers before they reach CI) |
| `pre-push`   | `git push`   | Runs `./scripts/ci-local.sh --quick` (the quality-only CI subset)                                 |

Bypass a single git action with `--no-verify` (e.g. `git commit --no-verify`), or
disable every hook for a shell session with `export TERMIHUB_SKIP_HOOKS=1`.
