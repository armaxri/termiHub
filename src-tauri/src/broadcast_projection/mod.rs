//! Broadcast-membership authority — Phase 4 step 5b of the stateless-UI
...
//! landed first ([`crate::restore_cohort_projection`], PR #2240), and the
//! workflow machine migrated as its own step.
//!
...
//! for broadcast membership. The live UI subscribes to the `broadcast` region and
//! frontend code dispatches `broadcast.*` intents; the former `appStore`
//! broadcast reducers were removed. The scope-to-tabs resolution, the
//! connected-terminal fan-out filter, and dynamic-membership recompute stay
//! frontend — they need the live tab tree the layout machine owns (see
//! [`store`]).
