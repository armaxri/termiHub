...
mod tests {
    ...
    // desktop crate's private `utils` module. Self-skips when the SSH container is
    // not up, mirroring the `require_docker!` convention in `core/tests/common`.
    // Bring the container up with:
    //   docker compose -f tests/docker/docker-compose.yml up -d ssh-password
    //
    // The container port is read from `TERMIHUB_TEST_SSH_PASSWORD_PORT` (default
    // 2201) so a sharded / parallel run can point at a separate container
    // instance, and every upload targets a UUID-suffixed remote path so
    // concurrent tests never collide on the remote `/tmp` file.

    /// Default host port of the `ssh-password` test container (`tests/docker`).
    const DEFAULT_SSH_PASSWORD_PORT: u16 = 2201;

    /// Resolve the SSH test-container port, allowing an env override so parallel
    /// runs can target distinct container instances.
    ...
    /// Returns `true` if the SSH test container accepts TCP connections.
    fn ssh_container_reachable(port: u16) -> bool {
    ...
    async fn agent_deploy_sftp_upload_round_trips_over_real_ssh() {
        ...
        if !ssh_container_reachable(port) {
            eprintln!(
                "SKIPPED: SSH container not reachable on port {port} \
