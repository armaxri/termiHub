...
use super::config::{
    MissedRunPolicy, Schedule, ScheduleAction, ScheduleRule, ScheduleRunOutcome,
    ScheduleRunResult, ScheduleStore, ScheduleTargets,
};
use super::storage::ScheduleStorage;
use super::timing::{next_run_after, validate_rule};
...
fn err(msg: impl Into<String>) -> TerminalError {
...
fn validate_input(input: &ScheduleInput) -> Result<(), TerminalError> {
...
fn aggregate(reports: &[WindowRunReport], catch_up: bool, at: DateTime<Utc>) -> ScheduleRunResult {
    ...
    let (outcome, message) = if ran.is_empty() {
        ...
        if let Some(failed) = worst(ScheduleRunOutcome::Failed) {
            let detail = failed.message.clone().unwrap_or_else(|| "a run failed".into());
            (ScheduleRunOutcome::Failed, Some(format!("{summary}: {detail}")))
        } else if worst(ScheduleRunOutcome::Cancelled).is_some() {
            (ScheduleRunOutcome::Cancelled, Some(format!("{summary}: cancelled")))
        } else {
...
fn skipped(at: DateTime<Utc>, message: impl Into<String>, catch_up: bool) -> ScheduleRunResult {
    ScheduleRunResult {
        at: at.to_rfc3339(),
...
impl ScheduleManager {
    ...
    fn view<Tz: TimeZone>(inner: &Inner, s: &Schedule, now: DateTime<Utc>, tz: &Tz) -> ScheduleView {
    ...
    /// Number of enabled schedules (for the status-bar indicator).
    pub fn enabled_count(&self) -> usize {
        self.inner
            .lock()
            .map(|i| i.store.schedules.iter().filter(|s| s.enabled).count())
            .unwrap_or(0)
    }
    ...
    pub fn report(
        ...
        let mut inner = self.lock()?;
        let inner = &mut *inner;
        let Some((id, rt)) = inner.runtime.iter_mut().find(|(_, rt)| {
            rt.active.as_ref().is_some_and(|a| a.token == token)
        }) else {
                ...
                if active.pending.is_empty() || stale {
                    let settled = if stale {
                        ScheduleRunResult {
                            ...
                            message: Some(
                                "No completion was reported within 6 hours".to_string(),
                            ),
